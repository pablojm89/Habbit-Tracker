...
function rewriteDenseTraining(ss, syncedAt, state) {
  ...
  const rows = entries.map((entry) => {
    ...
    return [
      ...
      holdSecondsPerRound || "",
      totalHoldSeconds || "",
      Number(entry.target_total_hold_seconds || 0) || "",
      holdSecondsPerMin || "",
      isometricCapacity || "",
      entry.rom_cm == null ? "" : Number(entry.rom_cm),
      load || "",
      entry.weight_per_dumbbell_kg || "",
      entry.dumbbell_count || "",
      added || "",
      Number(entry.assist_load_kg || 0) || "",
      bw || "",
      ...
      entry.effort_value || "",
      entry.readiness || "",
      Boolean(entry.is_test),
      Boolean(entry.failed),
      entry.missed_reps || "",
      entry.session_fatigue || "",
      entry.expected_comparison || "",
      entry.session_notes || "",
      entry.ladder_sequence_planned ? JSON.stringify(entry.ladder_sequence_planned) : "",
      entry.ladder_sequence_actual ? JSON.stringify(entry.ladder_sequence_actual) : "",
      workingPct || "",
      e1rm || "",
      capacity || "",
      effectiveLoad || "",
  ...
  writeTable(
    ss,
    SHEETS.dense,
    ["synced_at", "entry_id", "version", "schema_version", "date", "exercise_id", "exercise_name", "exercise_family_id", "variant_id", "nature", "scheme", "scheme_base", "scheme_target", "scheme_type", "duration_min", "target_reps_per_min", "target_total_reps", "total_reps", "reps_per_min", "hold_seconds_per_round", "total_hold_seconds", "target_total_hold_seconds", "hold_seconds_per_min", "isometric_capacity", "rom_cm", "external_load_kg", "weight_per_dumbbell_kg", "dumbbell_count", "added_load_kg", "assist_load_kg", "bodyweight_kg", "bodyweight_source", "total_system_load_kg", "visible_added_load_kg", "relative_strength", "effort", "effort_value", "readiness", "is_test", "failed", "missed_reps", "session_fatigue", "expected_comparison", "session_notes", "ladder_planned", "ladder_actual", "working_pct", "e1rm_kg", "bodyweight_capacity", "effective_load_kg", "tonnage_kg", "tut_load_kg_seconds", "bodyweight_contribution_pct", "tonnage_factor", "notes", "created_at", "updated_at", "deleted_at"],
    rows,
    true,
  );
}
