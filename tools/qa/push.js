...
(async () => {
  ...
  try {
    ...
    for (const minutes of [2, 5]) {
      ...
      check(await page.evaluate((minutes) => quickTimerState.running && quickTimerState.rounds === minutes && quickTimerState.scheme === `${minutes}D` && quickTimerState.context === null, minutes), `Reloj de ${minutes} minutos sin inventar marca`);
