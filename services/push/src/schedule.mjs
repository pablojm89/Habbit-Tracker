...
export function preferences(value) {
  ...
  const prefs = { times, timeZone: value.timeZone, equipment, kinds };
...
export function eligibleSessions(prefs) {
  return sessions.filter((session) => prefs.kinds.includes(session.kind) && session.equipment.every((item) => prefs.equipment.includes(item)));
}
...
export function chooseSession(prefs, previousId = "", random = Math.random) {
  ...
  const fresh = all.filter((session) => session.id !== previousId);
